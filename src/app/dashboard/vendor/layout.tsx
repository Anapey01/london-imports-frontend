...
export default function VendorDashboardLayout({
    ...
    return (
        <div className={`min-h-screen pt-16 md:pt-20 ${isDark ? 'bg-slate-950' : 'bg-gray-50'}`}>
            ...
            <main className={`transition-all duration-300 p-4 md:p-8 md:ml-72 ${mobileMenuOpen ? 'blur-sm md:blur-none' : ''}`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div>
                        <h1 className={`text-2xl md:text-3xl font-bold tracking-tight mb-1 ${isDark ? 'text-slate-50' : 'text-gray-900'}`}>
                            {user.vendor_profile?.business_name || 'Vendor Dashboard'}
                        </h1>
                        <p className={`text-sm md:text-base font-light ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>
