...
export default function OrderInvoicePage() {
    ...
    const handleSendInvoice = async () => {
        if (!isFullyPaid) {
            setSendFeedback({
                type: 'error',
                message: `Cannot send invoice: This order still has an unpaid balance of GH₵ ${balanceDueNum.toFixed(2)}. Invoices are only sent after full payment.`
            });
            return;
        }
        ...
        try {
            ...
            setSendFeedback({
                type: 'success',
                message: res.data?.message || `Invoice successfully sent to ${order.email}!`
            });
        } catch (err: unknown) {
            const errorMsg = (err as { response?: { data?: { error?: string; detail?: string } } })?.response?.data?.error 
                ...
                || 'Could not send invoice. Please make sure the order is fully paid.';
            setSendFeedback({
                type: 'error',
                message: errorMsg
            });
            ...
            <div className="max-w-[760px] mx-auto mb-6 print:hidden space-y-4">
                <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
                    <button 
                        ...
                        className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-slate-950 transition-colors py-1.5 px-2 rounded-lg hover:bg-slate-50 self-start sm:self-auto"
                    >
                    ...
                    <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                        <button 
                            ...
                            title={!isFullyPaid ? "Invoice can only be sent once the balance is GH₵ 0.00" : "Email invoice to customer"}
                            className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-sm ${
                                isFullyPaid 
                        ...
                        <button 
                            onClick={handlePrint}
                            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-950 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-slate-800 transition-all shadow-md cursor-pointer ml-auto sm:ml-0"
                        >
                    ...
                    <div className="p-4 rounded-xl text-xs bg-amber-50 border border-amber-200 text-amber-800 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                                This order has an unpaid balance of <strong>GH₵ {balanceDueNum.toFixed(2)}</strong>. The <strong>Paid in Full</strong> invoice can be sent once total payment is completed.
                            </p>
                        </div>
                    </div>
                )}
