...
const Footer = () => {
    ...
    // Check for vendor slug
    useEffect(() => {
        let isMounted = true;
        const slug = params?.slug as string;
        const isStoreRoute = slug && pathname?.startsWith('/store/');
        ...
        const fetchVendor = async () => {
            try {
                ...
                console.error('Failed to load vendor for footer', error);
                if (isMounted) {
                    setVendorBrand(null);
                }
            }
        ...
        if (isStoreRoute) {
            ...
            // Defer clearing to avoid synchronous render warning
            setTimeout(() => {
                setVendorBrand((prev) => (prev ? null : prev));
            }, 0);
        }
    ...
    return (
        <footer className="hidden md:block bg-slate-950 text-slate-400 py-16 border-t border-slate-900 relative overflow-hidden">
            {/* Ambient Background Glow - Adjust color for vendor if possible, currently using default pink */}
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
                <div className="grid md:grid-cols-12 gap-12 lg:gap-8 mb-16">
                    ...
                    <div className="md:col-span-4 lg:col-span-5">
                        <div className="flex items-center gap-3 mb-6">
                            {vendorBrand ? (
                                <>
                                    {vendorBrand.logo ? (
                                        <Image src={vendorBrand.logo} alt={vendorBrand.name} width={48} height={48} className="rounded-xl shadow-2xl ring-1 ring-white/10 w-12 h-12 object-contain bg-white" />
                                    ) : (
                                        <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-white font-bold text-xl ring-1 ring-white/10">
                                            {vendorBrand.name.charAt(0)}
                                        </div>
                                    )}
                                    <div>
                                        <span className="text-white font-bold text-lg tracking-tight block">{vendorBrand.name}</span>
                                        <span className="text-xs text-slate-500 font-medium">Partner Store</span>
                                    </div>
                                </>
                            ) : (
                                <>
                        ...
                        <p className="text-sm leading-relaxed text-slate-400 mb-6 max-w-sm">
                            {vendorBrand ?
                                (vendorBrand.description || `Browse our exclusive collection at ${vendorBrand.name}. Quality products delivered to your doorstep.`) :
                    ...
                    <div className="md:col-span-2 lg:col-span-2">
                        ...
                        <ul className="space-y-4 text-sm">
                            ...
                            {!vendorBrand && <li><Link href="/faq" className="hover:text-pink-400 hover:translate-x-1 transition-all inline-block">FAQs</Link></li>}
                    ...
                    <div className="md:col-span-3 lg:col-span-2">
                        ...
                        <ul className="space-y-4 text-sm">
                            {!vendorBrand && <li><Link href="/about" className="hover:text-pink-400 hover:translate-x-1 transition-all inline-block">Our Story</Link></li>}
                    ...
                    <div className="md:col-span-3 lg:col-span-3">
                        ...
                        <address className="text-sm not-italic space-y-3 mb-6">
                            ...
                            <a href="https://wa.me/233545247009" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-green-400 transition-colors group">
                ...
                <div className="pt-8 border-t border-slate-900 flex flex-col md:flex-row items-center justify-between gap-6">
                    <p className="text-xs text-slate-500">
                        &copy; {new Date().getFullYear()} {vendorBrand ? vendorBrand.name : "London's Imports"}. All rights reserved.
                    </p>
