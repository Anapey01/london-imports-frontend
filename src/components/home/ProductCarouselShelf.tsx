    ...
    return (
        <section className="w-full bg-white dark:bg-slate-900 border-y border-slate-100/60 dark:border-slate-800/50 py-8 my-4 relative overflow-hidden group/shelf">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Content: Clean Row when few items, Smooth Carousel when many items */}
                {isFewItems ? (
                    <div className="flex flex-wrap gap-4 sm:gap-6 px-1">
                        {products.map((product) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <div className="relative aspect-square w-full bg-slate-50/50 dark:bg-slate-900/50 rounded-xl p-2.5 flex items-center justify-center overflow-hidden border border-slate-100/80 dark:border-slate-800/60">
                                            <Image
                                                src={imageUrl}
                                                alt=""
                                                aria-hidden="true"
                                        ...
                                        <p className="text-xs sm:text-[13px] text-slate-800 dark:text-slate-200 font-normal mt-2 line-clamp-2 leading-snug group-hover:text-slate-950 dark:group-hover:text-white transition-colors min-h-[30px]">
                    ...
                    <div
                        ...
                        className="flex gap-4 overflow-x-auto scrollbar-thin scroll-smooth px-1 pb-2 no-scrollbar"
                    ...
                    >
                        {products.map((product) => {
                            ...
                            return (
                                <div
                                    key={product.id}
                                    className="w-[140px] sm:w-[170px] md:w-[190px] flex-shrink-0 scroll-snap-align-start"
                                >
                                    <Link
                                        href={`/products/${product.slug}`}
                                        className="group block"
