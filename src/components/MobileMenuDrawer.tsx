        ...
        <>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pb-10">
                        ...
                        <div className="px-4 space-y-1">
                            ...
                            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${categoriesOpen ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                <div className="pl-14 pr-4 pb-4 space-y-3">
                                    <Link href="/products" onClick={onClose} className="block text-sm font-bold text-pink-600 hover:text-pink-700 underline underline-offset-4">All Collections</Link>
                                    {categories.slice(0, 10).map((category: { id: string; slug: string; name: string }) => (
                                        <Link key={category.id} href={`/products?category=${category.slug}`} onClick={onClose} className="block text-sm text-gray-500 font-bold hover:text-gray-900 transition-colors">{category.name}</Link>
                                    ))}
                                </div>
                            </div>
                    ...
                    {/* Socials Section - Professional Restoration */}
