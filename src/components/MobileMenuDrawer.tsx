...
export default function MobileMenuDrawer({ isOpen, onClose, vendorBrand }: MobileMenuDrawerProps) {
        ...
        <>
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
                    <Link href={vendorBrand ? `/store/${vendorBrand.slug}` : "/"} onClick={onClose} className="flex items-center gap-2">
                        {vendorBrand ? (
                            <>
                                {vendorBrand.logo ? (
                                    <Image src={vendorBrand.logo} alt={vendorBrand.name} width={40} height={40} className="rounded-lg object-contain w-10 h-10" />
                                ) : (
                                    <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-store-primary font-bold shrink-0">
                                        {vendorBrand.name.charAt(0)}
                                    </div>
                                )}
                                <span className="text-lg font-bold text-gray-800 line-clamp-1">{vendorBrand.name}</span>
                            </>
                        ) : (
                            <>
                                <Image
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 -mr-2 text-gray-700 hover:text-store-primary rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-store-primary"
                ...
                {vendorBrand && (
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <span className="block font-medium text-gray-900 text-base">Store Home</span>
                        </div>
                        <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-gray-400" />
                    </Link>
                )}

                {/* Ready to Ship - Clean & Professional (Platform Only) */}
                {!vendorBrand && (
                    <a
                            ...
                            <>
                                ...
                                {user?.role === 'VENDOR' && (
                                    <Link
                                        ...
                                        onClick={onClose}
                                        className="flex items-center gap-4 px-6 py-3 w-full text-left hover:bg-pink-50 transition-colors text-pink-600"
                                    >
                ...
                <div className="mt-6 border-t border-gray-100 pt-6">
                    ...
                    <div className="pb-2">
                        ...
                        {!vendorBrand && (
                            <>
                                <Link href="/how-it-works" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                                    <HelpCircle className="w-5 h-5 text-gray-600" />
                                    <span className="text-gray-800">How It Works</span>
                                </Link>
                                <Link href="/delivery-returns" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                        ...
                        <Link href="/track" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                            <MapPin className="w-5 h-5 text-gray-600" />
                            <span className="text-gray-800">Track Order</span>
                        </Link>
                        {!vendorBrand && (
                            <Link href="/about" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                        ...
                        {vendorBrand ? (
                            <a href="#" className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 text-gray-700 font-medium transition-colors">
                                ...
                                <span className="text-gray-800">Contact Vendor</span>
                            </a>
                        ) : (
                            <a href="https://wa.me/233545247009?text=Hi%20I%20have%20a%20question%20about%20London's%20Imports" target="_blank" rel="noopener noreferrer" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                                <Phone className="w-5 h-5 text-gray-600" />
                        ...
                        {!vendorBrand && (
                            <Link href="/faq" onClick={onClose} className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors">
                ...
                {/* Trustpilot - External Link (Hide for Vendor?) */}
                {!vendorBrand && (
                    <a
