...
export default function Navbar() {
    ...
    return (
        <nav className="border-b sticky top-0 z-50" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16">
                    ...
                    <div className="hidden md:flex items-center justify-center flex-1 mx-8">
                        <div className="flex items-center space-x-8">
                            ...
                            {/* Potential future links: Help, etc. */}
                        </div>
                    </div>
                ...
                {mobileMenuOpen && (
                    <div className="md:hidden py-4 space-y-2">
                        <Link href="/products" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>Pre-orders</Link>
                        <Link href="/how-it-works" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>How It Works</Link>
                        <Link href="/about" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>About Us</Link>
                        {isAuthenticated ? (
                            <>
                                <Link href="/orders" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>My Orders</Link>
                                <Link href="/profile" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>Profile</Link>
                                <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="block py-2 text-red-600">Logout</button>
                            </>
                        ) : (
                            <>
                                <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="block py-2" style={{ color: 'var(--text-secondary)' }}>Login</Link>
                                <Link href="/register" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-pink-500 font-medium">Sign Up</Link>
                            </>
                        )}
                        {/* Theme Toggle for Mobile */}
                        <div className="flex items-center justify-between py-3 border-t mt-2" style={{ borderColor: 'var(--border-color)' }}>
